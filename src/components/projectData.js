// projectData.js
const projectList = [
    {
      title: "Sign Up Form",
      description:
        "This project is a user-friendly sign-up form built with React. Its includes form validation and responsive design to ensure a seamless user experience across different devices.",
      imageUrl: process.env.PUBLIC_URL + "/signup.jpg",
      link: "https://daniel130496.github.io/signup-form/",
    },
    {
      title: "Price Card",
      description:
        "This project is pricing plan card built with React. It includes monthly and yearly plan with toggle button to switch between the two plans.",
      imageUrl: process.env.PUBLIC_URL + "/pricecard.jpg",
      link: "https://daniel130496.github.io/price-card/",
    },
    {
      title: "Weather App",
      description: "This project is a simple Weather App built with React that allow users to search for weathher information by city",
      imageUrl: process.env.PUBLIC_URL + "/weather.jpg",
      link: "https://daniel130496.github.io/weather-app/",
    },
    {
      title: "To-Do List App",
      description: "TThis is a simple To-Do List application built with HTML, CSS, and JavaScript.",
      imageUrl: process.env.PUBLIC_URL + "/todo-list.jpg",
      link: "https://daniel130496.github.io/todo-list/",
    },
  ];
  
  export default projectList;
  